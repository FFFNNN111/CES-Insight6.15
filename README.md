...
- CES 感知倾向：使用独立按钮运行，本地数据集先给出候选 CES 类别和关键词命中，DeepSeek / MiMo 再给出倾向分值、依据和人工复核标记。
- AI 感知频率复核：使用独立按钮运行，本地数据集先给出关键词命中，DeepSeek / MiMo 再按 `PFj = Nj / N` 输出语义级命中片段、判断原因和人工复核标记。
