...
EXTRACT_DIR = ROOT / "literature_reports" / "_docx_extract"
OUTPUT = ROOT / "literature_reports" / "2026-06-01_two_paper_review.md"
...
def load_extract(stem: str) -> dict:
    return json.loads((EXTRACT_DIR / stem).read_text(encoding="utf-8"))


def doc_stats(data: dict) -> list[str]:
    tracked = data["tracked_changes"]
    return [
        f"- 正文段落：{data['paragraph_count']}；粗略字词数：{data['wordish_count']}。",
        f"- 批注：{data['comments_count']}；修订：插入 {tracked['insertions']}、删除 {tracked['deletions']}、移动 {tracked['moves']}。",
        f"- 交叉引用/字段：{data['field_count']} 个；未在提取文本中发现 `Error! Reference source not found`。",
        f"- 图片媒体：{data['media_count']} 个；脚注/尾注：{len(data['footnotes'])}/{len(data['endnotes'])}。",
        f"- 参考文献区段：提取到 {data['reference_count']} 段；其中包含正文编号文献和中文文献英文译文。",
    ]


def main() -> None:
    paper_one = load_extract("5_31基于大语言模型的城市生态系统文化服务供需评价与优化研究_以深圳市城市公园为例_1.json")
    paper_two = load_extract("百姓园林导向下城市公园生态系统文化服务感知供需研究_以深圳市为例.json")
    ...
    gb_refs = [
        ...
        "OPENAI. OpenAI GPT-4.5 system card[EB/OL]. (2025-02-27)[2026-06-01]. https://openai.com/index/gpt-4-5-system-card/.",
        "GUO D Y, YANG D, ZHANG H W, et al. DeepSeek-R1 incentivizes reasoning in LLMs through reinforcement learning[J]. Nature, 2025, 645: 633-638. DOI:10.1038/s41586-025-09422-z.",
        "LUO H, ZHANG Z, ZHU Q, et al. Using large language models to investigate cultural ecosystem services perceptions: a few-shot and prompt method[J]. Landscape and Urban Planning, 2025, 258: 105323. DOI:10.1016/j.landurbplan.2025.105323.",
        "ZHENG S W, REN Y Y, ZHU C Y, et al. Quantifying cultural ecosystem services in urban parks using social media and large language models: insights for Beijing's Garden City initiative[J]. Urban Forestry & Urban Greening, 2026: 129263. DOI:10.1016/j.ufug.2026.129263.",
        "ZHAO Z R, MA Z H, CHEN B Y, et al. Understanding public perceptions of cultural ecosystem services in urban coastal wetland ecological restoration areas: a social media-based large language model approach[J]. Journal of Environmental Management, 2026: 128816. DOI:10.1016/j.jenvman.2026.128816.",
        "GHERMANDI A, et al. Digital windows into nature's values: a critical review of cultural ecosystem services research with social media data[J]. Ecosystem Services, 2026: 101839. DOI:10.1016/j.ecoser.2026.101839.",
        "DANG A R, LI X. Supply-demand relationship and spatial flow of urban cultural ecosystem services: the case of Shenzhen, China[J]. Journal of Cleaner Production, 2023: 138765. DOI:10.1016/j.jclepro.2023.138765.",
        "TU X, HUANG G, WU J, et al. How do visitors' perceptions differ from the supply of cultural ecosystem services in urban parks? The case of Beijing[J]. International Journal of Sustainable Development & World Ecology, 2023. DOI:10.1080/13504509.2023.2234479.",
        "PARK S, et al. Operationalizing cultural ecosystem services in urban green planning: a systematic review[J]. Frontiers in Sustainable Cities, 2026. DOI:10.3389/frsc.2026.1768123.",
        "ZHANG X, ZHAO Y, HU Y. Assessing perception and equity of cultural ecosystem services in urban parks using social media data[J]. Scientific Reports, 2025. DOI:10.1038/s41598-025-18288-0.",
        "百度智能云. NLP-Python-SDK：情感倾向分析[EB/OL]. [2026-06-01]. https://cloud.baidu.com/doc/NLP/s/tk6z52b9z.",
        "百度智能云. 情感倾向分析_情感倾向分析算法[EB/OL]. [2026-06-01]. https://cloud.baidu.com/product/nlp_apply/sentiment_classify.",
        "HONG J Y, JEON J Y. Designing sound and visual components for enhancement of urban soundscapes[J]. The Journal of the Acoustical Society of America, 2013, 134(3): 2026-2036. DOI:10.1121/1.4817924.",
    ]

    insertion_rows = [
        ["论文一", "引言中 CES 评价与网络评论研究缺口段后", "补强社交媒体 CES 研究脉络和深圳案例基础", "Ghermandi 2026；Dang & Li 2023；Tu 等 2023；Zhang 等 2025", gb_refs[6] + "；" + gb_refs[7] + "；" + gb_refs[8] + "；" + gb_refs[10], "近作直接讨论社交媒体 CES、城市公园供需、深圳 CES 空间流动与感知公平。", "出版社/DOI 元数据；部分仅基于摘要/元数据判断"],
        ["论文一", "1.3.1 百度情感分析段 94-95", "替换“准确率较高”的弱支撑，改为官方接口参数来源", "百度智能云官方文档", gb_refs[11] + "；" + gb_refs[12], "官方文档可支撑 sentiment、confidence、positive_prob、negative_prob 等参数；不能直接证明本文公园评论准确率。", "官方网页"],
        ["论文一", "1.3.2 大语言模型识别段 158-159", "补强 GPT-4.5、Few-shot、DeepSeek-R1 和 LLM+CES 方法依据", "OpenAI 2025；Brown 2020；Guo 等 2025；Luo 等 2025；Zheng 等 2026；Zhao 等 2026", gb_refs[1] + "；" + gb_refs[0] + "；" + gb_refs[2] + "；" + gb_refs[3] + "；" + gb_refs[4] + "；" + gb_refs[5], "这些文献分别支撑模型来源、少样本提示、推理型模型、CES 识别方法和同类最新应用。", "官方网页/出版社/DOI 元数据"],
        ["论文一", "讨论或局限性段 360 附近", "说明平台偏差、代表性和算法解释限制", "Ghermandi 2026；Zhang 等 2025", gb_refs[6] + "；" + gb_refs[10], "社交媒体数据不能等同全体公众，应说明样本偏差和公平性。", "出版社/DOI 元数据；部分仅基于摘要/元数据判断"],
        ["论文一", "参考文献 [28] 或声音/沉浸体验语境", "修正作者名并限定用途", "Hong & Jeon 2013", gb_refs[13], "原文 `Young J H, Yong J J` 不是规范作者写法；该文只适合支撑声景/视觉组件，不宜泛化到公园类型差异。", "DOI 元数据"],
        ...
        ["论文二", "段 16-17 CES 与供需关系综述后", "补强供需、规划和社交媒体 CES 研究", "Dang & Li 2023；Tu 等 2023；Ghermandi 2026；Park 2026", gb_refs[7] + "；" + gb_refs[8] + "；" + gb_refs[6] + "；" + gb_refs[9], "与城市公园 CES 供需、感知差异和规划应用直接相关。", "出版社/DOI 元数据；部分仅基于摘要/元数据判断"],
        ["论文二", "1.3.1 段 125/158", "替换百度 API 准确率表述", "百度智能云官方文档", gb_refs[11] + "；" + gb_refs[12], "官方文档能证明接口和返回参数，不能替代本文本地验证集准确率。", "官方网页"],
        ["论文二", "1.3.2 段 189-190", "补强 LLM 识别、少样本提示和同类 CES 应用", "OpenAI 2025；Brown 2020；Guo 等 2025；Luo 等 2025；Zheng 等 2026；Zhao 等 2026", gb_refs[1] + "；" + gb_refs[0] + "；" + gb_refs[2] + "；" + gb_refs[3] + "；" + gb_refs[4] + "；" + gb_refs[5], "支撑模型、提示方法、推理模型和 CES 语义识别的合法来源。", "官方网页/出版社/DOI 元数据"],
        ["论文二", "局限与展望段 407-410", "补充代表性、平台偏差和规划转化边界", "Ghermandi 2026；Zhang 等 2025；Park 2026", gb_refs[6] + "；" + gb_refs[10] + "；" + gb_refs[9], "帮助把“百姓园林”价值判断与数据偏差、规划应用边界分开。", "出版社/DOI 元数据；部分仅基于摘要/元数据判断"],
    ]
...
{chr(10).join(doc_stats(paper_one))}
...
{chr(10).join(doc_stats(paper_two))}
...
|---|---|---|---|---|---|---|
{chr(10).join("| " + " | ".join(row) + " |" for row in insertion_rows)}
...
{chr(10).join(f"{i + 1}. {ref}" for i, ref in enumerate(gb_refs))}
